...
# Use device local overlay Android manifest if exists
ifneq (,$(findstring $(strip $(call my-topdir)),$(LOCAL_PATH)))
  LOCAL_MANIFEST_FILE := $(strip $(call my-topdir))/$(subst $(strip $(call my-topdir)),$(TARGET_DEVICE_DIR)/overlay,$(LOCAL_PATH)/AndroidManifest.xml)
else
  LOCAL_MANIFEST_FILE := $(strip $(call my-topdir))/$(TARGET_DEVICE_DIR)/overlay/$(LOCAL_PATH)/AndroidManifest.xml
endif

ifneq (,$(wildcard $(LOCAL_MANIFEST_FILE)))
  $(info Package modules have device local overlay Android manifest)
else
...
full_android_manifest := $(LOCAL_MANIFEST_FILE)
